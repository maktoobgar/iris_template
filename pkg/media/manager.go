...
type MediaManager interface {
	...
	GoTo(folderName string, createPath ...bool) MediaManager
	...
	GetHostAddress(mainMediaManager MediaManager, fileName string) string
}
...
func (m *mediaManager) GoTo(folderName string, createPath ...bool) MediaManager {
	return NewMediaManager(m.JoinPath(folderName), createPath...)
}
...
func (m *mediaManager) GetHostAddress(mainMediaManager MediaManager, filename string) string {
	return m.TrimEnd(strings.Replace(m.address, mainMediaManager.GoBack().GetAddress(), "", 1)) + "/" + filename
}
